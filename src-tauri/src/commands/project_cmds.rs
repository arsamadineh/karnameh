...
#[tauri::command]
pub async fn create_project(pool: State<'_, SqlitePool>, input: CreateProjectInput) -> Result<Project, AppError> {
    project_repo::create_project(&pool, input).await
}

#[tauri::command]
pub async fn update_project(pool: State<'_, SqlitePool>, input: UpdateProjectInput) -> Result<Project, AppError> {
